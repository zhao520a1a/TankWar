...
public class Missile {
	...
	public static final int WIDTH = 12;//因为所有子弹的方向图片已经大小剪裁一致了；
	public static final int HEIGHT = 12;
	
	private boolean live = true;
	private boolean good; //子弹也分好坏；
	...
	public boolean hitTank(Tank t){
		 //活着的子弹 &&相交（getRect().intersects(---)是为了判断子弹和坦克是否相交）&& 坦克也活着 && 子弹不是一伙的
		if(this.live && this.getRect().intersects(t.getRect()) && t.isLive() && this.good != t.isGood()){		 
			if(t.isGood()) {
				t.setLife(t.getLife() - 20);//主战tank每被打一次掉20血；
				if(t.getLife() <= 0) {
					t.setLive(false);
				}
			}else {
